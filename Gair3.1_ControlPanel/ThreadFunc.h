...
DWORD WINAPI UIProcessor(LPVOID pm)
{
	...
	Mat UI_Rsc=camFrame.clone(), textROI;


	// blur(textROI, textROI, Size(3, 3));
	while (!exitThread)
	{
		UI_Rsc = camFrame.clone();
		textROI = UI_Rsc(Rect(0, 0, ROI_w, ROI_h));
